			...
			for (int i = 0; i<div; i++) {
				...
				for (long tm : times) {
					if ((tm >= strttime) && (tm <= endtime)) {
						DumpinLogFile(strttime, endtime, _timeStampTagMap.get(tm));
...
//			_actualcount.clear();
	...
	private void DumpinLogFile(long start, long end, Set<String> hashTags) {
...
//					System.out.println(" TAG : "+hshtg+" : COUNT : "+_calculatedcount.get(hshtg)+" : ACTUAL : "+_actualcount.get(hshtg));
		...
		List<Integer> Secs10ranks = new ArrayList<Integer>(_rankTagMap.keySet());
		Collections.sort(Secs10ranks, Collections.reverseOrder());
		
		
		for (int rnk : Secs10ranks) {
			for (String hshtg : _rankTagMap.get(rnk)) {
				System.out.println("SORTED : TAG : "+hshtg+" : COUNT : "+_calculatedcount.get(hshtg)+" : ACTUAL : "+_actualcount.get(hshtg));
				try {
					_fw.write("SORTED : TAG : "+hshtg+" : COUNT : "+_calculatedcount.get(hshtg)+" : ACTUAL : "+_actualcount.get(hshtg));
					_fw.write(System.lineSeparator());
					_fw.flush();
				} catch (Exception e) {
...
//		bucketHashTags = new ArrayList<BucketpacketInfo>();
		...
		try{
			_fw = new PrintWriter(new File("/s/chopin/k/grad/amchakra/TweetLossyCountBolt.txt"));
		}catch (Exception e) {
